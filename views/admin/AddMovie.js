...
export function renderAddMoviePage(container, movieToEdit = null) {
    ...
    /** @tweakable [Styling for the delete button in admin item lists] */
    const deleteButtonStyle = "padding: 5px; border: 1px solid #ff0000; border-radius: 5px; color: #fff; background: #ff0000; cursor: pointer;";
    const content = `
        <div class="card">
                ...
                ${db.movies.map(movie => `
                    <li>
                        ...
                        <div class="actions">
                            ...
                            <button class="btn-danger" style="${deleteButtonStyle}" onclick="deleteItem('${movie.id}', 'movies')">Excluir</button>
                        </div>
                    </li>
                `).join('')}
            </ul>
